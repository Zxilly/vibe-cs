...
const SAMPLE: readonly { variant: AlertVariant; message: string; action: string; role: string }[] = [
  { variant: 'info', message: '分析完成后会在这里提示，可直接进入工作区', action: '知道了', role: 'status' },
  { variant: 'success', message: '视频已生成 · Kael_Mirage_1v3.mp4', action: '播放', role: 'status' },
  { variant: 'warning', message: '运动镜头缺少碰撞数据，可能穿墙', action: '改为 POV', role: 'alert' },
  { variant: 'danger', message: '导出未完成：磁盘空间不足，已保留工程与素材', action: '重试', role: 'alert' },
];

describe('Alert', () => {
  it.each(SAMPLE)('renders the $variant sample with its recovery action', (sample) => {
    const markup = renderMarkup(
      <Alert variant={sample.variant} action={{ label: sample.action, onAction: noop }}>
        {sample.message}
      </Alert>,
    );

    expect(markup).toContain(`data-tone="${sample.variant}"`);
  ...
  it('paints each tone from the §3.1 surface / border pair', () => {
    ...
    for (const sample of SAMPLE) {
      ...
      expect(markup).toContain(expected[sample.variant]);
    }
  });
  ...
  it('omits the dismiss control unless the caller supplies one', () => {
    const withoutDismiss = renderMarkup(
      <Alert variant="info" action={{ label: '知道了', onAction: noop }}>
        分析完成后会在这里提示
      </Alert>,
    );
    const withDismiss = renderMarkup(
      <Alert variant="info" action={{ label: '知道了', onAction: noop }} onDismiss={noop}>
        分析完成后会在这里提示
      </Alert>,
    );
  ...
  it('does not float above the page — a Notice is a block in the flow', () => {
    const markup = renderMarkup(
      <Alert variant="danger" action={{ label: '重试', onAction: noop }}>
        导出未完成
      </Alert>,
    );
