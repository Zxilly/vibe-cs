...
function cacheMetadata(value: unknown): ReplayCacheMetadata {
  ...
  const cache = value as ReplayCacheMetadata;
  if (!['hit', 'generated', 'bypassed'].includes(cache.state) || !Number.isSafeInteger(cache.version)) throw new Error(msg("m0380"));
  return cache;
}

export function decodeReplayBinary(buffer: ArrayBuffer): ReplayPayload {
  const reader = new Reader(buffer);
  if (new TextDecoder().decode(reader.raw(4)) !== 'ARPL' || reader.u16() !== 1) throw new Error(msg("m0151"));
  ...
  if (frameCount > 500_000) throw new Error(msg("m0377"));
  const frames: ReplayFrameRecord[] = [];
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    const tick = reader.u64();
    const playerCount = reader.u16();
    if (playerCount > 128) throw new Error(msg("m0311"));
