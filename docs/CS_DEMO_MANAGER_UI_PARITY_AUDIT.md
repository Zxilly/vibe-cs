...
- Vibe CS 代码基线：exact product build `fbc9c6a2c80fb44099bb62eec6b7e7b322afb58d` / tree `ef344b8d699c2088f66ff0db2c3f77066437f31b`（producer-bound Objective Review，包含此前 persistent truthful Player directory、Man Advantage、Activity exact context 与 stale action receipt 修复）；文档更新单独记录，不冒充产品构建输入
...
Vibe CS 目前还不是 CS Demo Manager 的完整替代品。它已经补上跨三场 Major 的 Evidence Search、exact player-involvement 查询、绑定 canonical evidence 的持久复盘注释与全局注释索引、Highlights 对同一注释的摘要/CRUD 复用、单场 Player Atomic Evidence、全局 Player profile 的 first-10 持久证据预览、SQLite 持久 Player-match projection 上的全局筛选/稳定排序/分页/coverage、最多两个 URL-owned 显式 ID 的 exact compare、Clutch Review、带 10×10 方向对位矩阵的 Openings、基于逐回合 5v5 roster 真值的 Team Round、Team Economy、Man Advantage Review 与 producer-bound Objective Review、SQLite 边界分页和 exact kind/UUID detail 的 Activity Center、current-only durable Analysis Run/bounded events/exact producer result、Production exact task deep link、只重放可证明 unpublished suffix 的 recording retry，以及 Library 服务端查询、power table/Inspector、真实 URL 列选择和最多 12 个显式 ID 的批量 Analysis 选择。既有产品库已索引 M1/M2/M3 共 11,548 条证据并报告 `scan_complete=true`；Player fresh gate 另证明 10 人、projection coverage `3/3`、三条日期未知但目录时间单独标注的本地比赛明细，以及 single/pair URL Reload/Back。最新 Objective fresh gate 只分析 M1，证明 8/8 plant rounds 与双尺寸工作台；三场 34-round 结果另由只读 SQLite oracle 支持。剩余差距集中在持久团队实体、尚缺的高级指标/矩阵、尚未覆盖的 review/taxonomy 消费端、连续高保真回放，以及真实 CS2 + HLAE 成片与 retry 门禁：
...
8. durable Analysis Run 的 cancel/heartbeat、并发 retry 与其他 failure 形态；completed 及一次真实 startup interruption→new-run retry 已过 fresh Tauri/真实 Major 门禁。
...
旧 Activity gate 的几何以及旧 Player 三卡/链接/双尺寸布局证据保持有效，但旧 `analysisrun-audit` 的卡片标签和 Inferno/Anubis/Mirage 顺序不作为比赛日期证据；日期真值只使用 Player projection gate。新的 Activity gate 已补一次真实 startup interruption→retry，并验证 exact URL 选择独立于 filter 和 reload。它仍不能证明 Activity 多页/性能、Analysis cancel/concurrent retry、其他 kind 的真实 mutation、Steam/recording retry、录制、下载、导出、Watch、CS2、HLAE 或模型生成成功态。Player gate 也未启动 Watch/CS2/HLAE，未执行真实 Steam 网络下载，不能把内容寻址与下载并发 TDD 写成外部下载验收。Man Advantage 只证明 remaining-uneliminated state stream；不推断 health/alive/disconnect、win probability、trade、KAST 或 rating。Objective fresh visual 只证明 analyzed M1；三场 34 verified plant rounds、种包方 `29/5`、terminal `5/16/13`、80 kill atoms 与 247 damage-event atoms 来自独立 deterministic、environment-gated、read-only SQLite audit，不能冒充 M2/M3 fresh visual。它不推断 site A/B/空间、retake/save/trade/KAST/rating，且未点击 Round/Replay/Watch/Add 或启动 CS2/HLAE。隔离运行不是旧默认数据库的迁移或兼容。选项二参考与真实 Context Canvas 的等尺寸归一化并排证据仍为 `target/parity-continuation-20260813/13-option2-vs-real-context-normalized.png`；它不冒充同一原生 viewport。CSDM 截图来自 v3.20.1 exact commit 的官方仓库预览和当前官方文档。所有 `target` 目录只作为本地审计证据，不进入 Git。
...
| 回合与玩家证据 | CSDM Rounds、Match Players 与 Player profile 源码 | `08-evidence-round-link-1100x700.png`、`19-rounds-context-canvas-compact-nav-1100x700.png`、`product-audit-20260813-current/05-openings-matrix-max-2560x1392.png`、`06-openings-filtered-karrigan-fallen-max.png`、`07-openings-filtered-1100x700.png`、`target-workflow-audit-20260813/07-players-max-evidence.png`、`12-players-1100-drawer-evidence.png`、`target-playerprojection-audit-20260814/visual/screenshots/01-players-max-profile-top.png`、`02-players-max-match-history.png`、`03-players-1100-profile-drawer.png`、`04-players-1100-pair-drawer.png` | 单场原子证据与 Openings 既有门禁保持。fresh current FalleN `76561197960690195` profile 来自 coverage `3/3` 的持久 Player-match projection，显示三条 exact local rows；三场 `match_date=NULL`，页面逐条显示“比赛日期不可用”并另标 catalog time。single/pair URL Reload 与 Back、最大化和 600×700 窄窗 Drawer 均通过，无 document overflow；跨比赛 Evidence Index 为 `indexed_demos=3`。旧 `analysisrun-audit` Player 截图只作历史三卡/链接/布局证据。仍不能写成真实多页、大库或 multi-match 图表通过。Watch 未点、CS2 未启动。 |
...
| Activity | CSDM Analyses/Downloads 队列 | `target-manadv-activity-audit-20260814/screenshots/04-activity-exact-max.png`、`05-activity-exact-1100.png`、`06-activity-interrupted-max.png`、`07-activity-retry-completed-max.png`、`08-activity-final-no-stale-receipt.png` | exact kind/UUID row 独立于 list filter/page，Production deep link、filter-out 与 reload 保持已过。真实 M3 host-stop 后 run `4ded…` 成为 interrupted；retry 创建 distinct completed `02a7…`，旧 run 保留。截图 01–07 属于 `2d73e7f`；`fc80c5f` 截图 08 只证明 completed exact load 无 stale receipt，same-session 自动退役由 TDD/focused 24 证明。仍未验 cancel、其他 kind mutation、多页或性能。 |
...
| 分析任务中心 | 独立 Analyses 页，Pending/Analyzing/Inserting/Done/Error、日志、See Demo/Match | 独立 Activity 聚合 analysis/download/recording/export 的权威持久状态与真实动作，并支持 search/kind/state/分页、exact kind/UUID detail；Analysis 有 exact run/stage/error/fingerprint/events/result/retry | 部分 | summary/filter/page 同 snapshot；exact row 另在单一 transaction 读权威 source 与 retry/result capability。真实 completed、Production/filter/reload exact selection、一次 startup interrupted→distinct retry completed 均已过。仍没有 cancel/heartbeat，并发 retry、其他 kind mutation、多页/性能未验。 |
...
| 全局玩家目录 | 表格列、筛选、标签、评论、封禁、XLSX；个人 Overview/Charts/Maps/Heatmap/Rank/Matches | 持久 SQLite Player-match projection、服务端搜索/分页/稳定排序、coverage、最多两个 URL-owned ID 的 ordered exact compare、bounded aliases + total、汇总、头像、本地比赛明细、Steam 公共资料、first-10 exact involvement evidence | 部分 | fresh FalleN profile 为 projection `3/3`、三条 `match_date=NULL` 的 exact local rows 与 `indexed_demos=3`；每行显示比赛日期不可用并另标 catalog time，链接保留 exact player+Demo。FalleN/NiKo pair、max Inspector、1100 Drawer 与 Reload/Back 均已过。目录仍只有一页，不能外推真实多页/大库或 multi-match 图表；仍缺个人图表、地图、热图、段位历史、评论、标签、封禁和导出。 |
...
- 当前 Analysis 已以 `analysis_runs` 和 `analysis_run_events` 持久每次尝试。状态为 `queued/running/completed/failed/interrupted`；固定阶段覆盖 input validation、parser queue/start、parse 后 source revalidation、projection 与 terminal state。每 run 最多 32 个顺序事件，error/detail 最多 2,000 字符，不生成 percentage。
...
- Activity 使用 `analysis:<run_id>`，Inspector 显示 exact stage、SHA-256/size、persisted error、result availability 与 ordered events。eligible latest failed/interrupted attempt 可以创建新 run retry，旧 attempt 不被改写；completed 只有 exact producer result 存在时才能 Open Analysis。URL 同时保留 `demo` 与 `run`。
...
1. completed run/Inspector 与一次 startup interruption→distinct retry completed 已经过 fresh Tauri/WebView2 + 真实 Major 门禁；其他 failure、cancel 与 concurrent retry 尚未实跑。
2. 没有 Analysis cancel、heartbeat/lease 或用户可配置并发数；当前全局单 parser admission 更偏安全策略，界面未解释。
...
- 严格 Player URL 拥有 `q/page/sort/direction/player/compare/matches_page/inspector=1`。最多两人 ordered exact compare、single/pair Inspector 与 match page 都可经 Reload/Back 恢复；未知/重复/不一致参数 fail closed。
...
| 状态 | queue badge、analysis logs 明确 | in-context Notice、真实 disabled reason、独立 Activity Center、exact Analysis run events | Vibe 仍需要全局 badge、Analysis cancel/heartbeat 与 download/export/recording 更完整的持久日志。 |
...
- Library 已把宽屏空白转换为 power table + 逐条 Inspector/drawer，并加入服务端 query window、当前 URL 列选择和 explicit batch Analysis selection；Players 也已把目录移到 persistent SQLite projection，以 coverage、truthful nullable match date、profile/history 与 compare Inspector/drawer 承载真实数据，并在 profile 显示 first-10 involvement evidence。M1/M2/M3 三条 exact local rows、FalleN/NiKo aggregate、双尺寸 Drawer 与 URL Reload/Back 已过；两者仍缺真实多页、大库性能和 multi-match 图表门禁。
...
3. **深化 Activity Center**
   ...
   已聚合 analysis/download/record/export 的权威持久事实和真实动作，并在 SQLite transaction 内提供 summary、search/kind/state、filtered total 与 stable page；exact kind/UUID endpoint 从单一权威来源同 transaction 计算 retry/result capability。URL selection 独立于 list filter/page，strict response binding、last-good stale/backoff/404 unavailable、abort guard、Production deep link 与 mutation lease 已落。真实 M1 已过 Production/filter/reload exact selection 和双尺寸；真实 M3 已过一次 startup interrupted→distinct retry completed，且旧 run 保留。`fc80c5f` 修复 stale queued receipt；final 视觉只确认 completed exact load 为 `receipt=[]`，same-session 自动退役由 TDD/focused 24 tests 证明。下一步是真实多页/大库性能、其他 kind mutation、Steam/recording failure-retry、Analysis cancel/heartbeat/并发 retry 与通用 pause/resume。Agent mutation 仍需单独确定持久来源。
...
6. **逐玩家广度与队伍实体**
   ...
   Analysis completion 同事务维护 persistent Player-match projection；SQLite server filter → stable sort → count → paginate、coverage 与 exact absence semantics 已落，不再运行时扫描最多 1,000 个 Analysis 文档。最多两个 URL-owned ID 的 ordered compare、single/pair Drawer、Reload/Back 已过；profile 另以 exact involvement query 显示 first-10 persistent evidence、total/index completeness 与 Round/Replay/full-search。current FalleN 为 projection `3/3`、三条比赛日期明确不可用的 local rows 与 `indexed_demos=3`；最大化和 1100×700 均无 document overflow。继续补真实目录多页/大库、multi-match charts/maps/heatmap，并建立 Team continuity、maps、matches 和 heatmap。
...
本轮最新 accepted set 位于 `target-objective-audit-20260814/visual/screenshots/`：`01-objective-max.png`、`02-objective-1100.png` 与 `03-objective-1100-inspector.png`，结构化证据为 `target-objective-audit-20260814/visual/evidence.json`。三张均来自 exact HEAD `fbc9c6a2c80fb44099bb62eec6b7e7b322afb58d` / tree `ef344b8d699c2088f66ff0db2c3f77066437f31b` / EXE SHA-256 `ab3a18edd0993e2d8f1920dea40f9e30f32cf1b9582e01bf5e71c2f82a4f7c29` / identifier `app.vibecs.objective-audit` 的同一 fresh product flow；worker SHA-256 为 `2e99e8e365b7047dcd39eebc305d79e84438ea7d58757e2fb1eed4cb14c87255`。上一组 Player accepted set 位于 `target-playerprojection-audit-20260814/visual/screenshots/`，来自 exact HEAD `1f7397ec857dc592d4e8525fc9ac4bf299d34db7` / EXE SHA-256 `792eca8491d4ae36dfbfcc5ff3c9fed322edd1d72e58e0e7dc1abb85e1bfad01` / identifier `app.vibecs.playerprojection-audit`，不能与 Objective build 混写。更早 `target-manadv-activity-audit-20260814/screenshots/` 的 01–07 来自 `2d73e7f` 初始 build，08 来自 `fc80c5f` final focused build，不能把两组冒充同一 executable，也不能把 08 写成第二次 same-session retry。旧 durable-run accepted set 仍位于 `target-analysisrun-audit-82617d4/screenshots/`，其中 Player 截图只保留三卡/链接/布局意义，不再证明比赛日期；更早失败基线位于 `target/visual-audit-20260813-0805-tauri/`。这些目录为本地审计证据，不进入 Git。
...
- fresh DB 只有 3 条 Library 记录；M2+M3 explicit selection 与排序保留是旧实机观察，但 Library 跨页保留只有 TDD，真实多页/大库性能未覆盖。Player gate 已证明 10 人、projection coverage `3/3`、FalleN 三条 exact local rows、FalleN/NiKo pair、URL Reload/Back 与双尺寸无 overflow；三场 `match_date` 全为 `NULL`，所以只证明明确的未知日期和单独 catalog time，不证明比赛日期或按比赛时间排序。旧 `analysisrun-audit` Player 截图仅证明三卡/链接/布局。Team Economy 只证明 M1 decoded purchase/cost/item；Man Advantage 只证明 roster minus death targets 的 21/21 state stream，不证明 health/alive、equipment value、economy type、win probability、trade、KAST 或 rating。Objective fresh gate 只分析 M1 并证明 8/8 plant-to-round-end UI；M2/M3 仅 discovered。三场 34 verified、种包方 `29/5`、terminal `5/16/13`、80 kill atoms/247 damage-event atoms 来自独立 environment-gated read-only SQLite audit，不是 fresh visual。raw site 不证明 A/B/空间，post-plant atoms 不证明 retake/save/trade/KAST/rating；alias/boundary/non-plant/action 反例来自 TDD。Activity 已覆盖 completed、exact filter/reload selection 与一次 analysis interruption→retry，但仍不能证明多页/性能、Analysis cancel/concurrent retry或其他 kind mutation；`recording + failed` 是空态，因此 Steam/recording retry 均未跑。Highlight annotation、Match History 与 Queue 的既有边界不变。Round/Replay/Watch/Add 未在最新 Objective 检查中点击；Watch、CS2、HLAE、录制、真实 Steam 下载和导出 mutation 均未在最新检查中触发。
- durable Analysis Run 的 completed success path、一次 startup recovery 为 `interrupted` 以及 distinct new-run retry completed 已在 fresh identifier/DB、同源 desktop/sidecar 与真实 Major 上完成产品验收；旧 run 没有被改写。当前仍没有 Analysis cancel/heartbeat；没有覆盖 concurrent retry、hard crash sidecar/process artifact cleanup，也没有消除 source-path pre/post fingerprint 的窄 physical-file TOCTOU，terminal-tail repair copy provenance 仍不在 run event 中。`fc80c5f` final screenshot 只确认 completed exact load 为 `receipt=[]`；same-session 自动退役由确定性 TDD/focused tests 证明，不是重复视觉流程。
...
- 默认 identifier 下的旧实验数据库被 exact current schema 拒绝；最新检查使用独立 `app.vibecs.objective-audit` identifier 与 fresh DB。这是测试隔离，不是兼容迁移。
