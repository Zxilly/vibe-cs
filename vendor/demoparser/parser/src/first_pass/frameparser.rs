    ...
    pub fn try_find_beginning_of_a_frame(demo_bytes: &[u8], start: usize, end: usize) -> Result<usize, DemoParserError> {
        ...
        */
        ...
        loop {
            ...
            if let Ok(f) = frame {
                if let Some(next) = f.frame_ends_at.checked_add(f.size) {
                if let Ok(inner) = FrameParser::read_frame(demo_bytes, next) {
                    if let Some(next) = inner.frame_ends_at.checked_add(inner.size) {
                    if let Ok(inner2) = FrameParser::read_frame(demo_bytes, next) {
                        if let Some(next) = inner2.frame_ends_at.checked_add(inner2.size) {
                        if let Ok(inner3) = FrameParser::read_frame(demo_bytes, next) {
