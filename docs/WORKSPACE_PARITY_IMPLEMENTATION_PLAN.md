...
本切片完成不等于全功能对齐。Evidence Search 已在既有真实产品库中覆盖 M1/M2/M3 共 11,548 条证据、50 行默认页、`scan_complete=true`、唯一 FalleN R20 结果及 Round/Replay 深链；实际 Watch 启动和投影重建仍未过。Player 原子证据、Weapons、Heatmap 点选、Library power table/Inspector 与 SQLite 侧 search/filter/stable-sort/page、只含真实数据的 URL 列选择、SQLite 持久 Player-match projection 上的 server filter → stable sort → paginate、最多两个显式 ID 的 Player compare、SQLite 边界分页 Activity、canonical Evidence Annotation 与全局 index、Clutch Review、Openings、roster-verified Team Round、Team Economy 与 Man Advantage Review 已落。本轮进一步加入最多 12 个显式 Demo ID 的 Library 批量 Analysis 选择、Evidence Search 的 exact player-involvement 查询、Player profile 的 first-10 跨比赛证据预览、Highlights 对同一 canonical annotation 的摘要与 CRUD drawer 复用、只重放可证明未发布后缀的 recording retry，以及 current-only 的 durable Analysis Run、bounded events、exact producer result、Activity exact task context/observer/retry 与 Production exact deep link。
...
`manadv-activity-audit` 已以真实 M1 验证 Production exact deep link、filter-out 保持、reload 保持及 2560×1392/1100×700 Activity 布局；又以真实 M3 验证一次 host-kill 后 startup `interrupted` 与一次 `retry_analysis` 新 run completed，旧 run 保留。该实机 gate 没有执行 Analysis cancel、其他 kind 的真实 mutation、Steam/recording retry、下载、导出、录制、CS2 或 HLAE。failed/cancelled Steam download 仍只有 latest eligible + owner/config 双重校验的 TDD；Recording retry 仍只允许 proven unpublished suffix durable child，也只有 TDD。Activity 多页/大库性能、download/export/recording 完整日志、通用 pause/resume 和真实 HLAE active-cancel/成片仍未闭合。
...
mutation 结果只在仍持有 selection lease 时生效。`retry_analysis` 返回的新 run ID/status/stage 会立即切换 exact URL，但旧 attempt 不修改；当该 exact run 的权威状态/stage 已前进，界面会退役旧 queued action receipt。这个规则只比较同一个 activity ID，不会因另一任务变化而清掉当前回执。
...
输入 provenance 只证明在 parse 前后分别从 Demo source path 观察到相同 SHA-256/size，并与当前 Demo record 一致；它不证明同一 open handle 的每个 parser-consumed byte。terminal-tail recovery 实际解析 bounded repair copy，而 copy 自身的 fingerprint/lineage 不在 run event 中，仍有窄的 physical-file TOCTOU。旧 `analysisrun-audit` 已用 fresh Tauri/WebView2 + 真实 M1/M2/M3 证明三个 completed run；其中 M1 的 exact six-event sequence、fingerprint/size、producer result、Analysis URL 与 Activity/Open link run identity 已通过。新的 `manadv-activity-audit` 又证明真实 M3 run `4ded8b20-59e9-4e14-961e-e2e44995e497` 在 host 被停止后于下一次启动成为 `interrupted`，retry 创建 distinct run `02a7df9d-6a5c-456c-baac-6a461fb4e200` 并 completed，旧 run 保持不变；Production exact deep link、filter-out/reload selection 与 Activity 双尺寸也通过。它仍不证明 cancel、concurrent retry、physical sidecar cleanup、多页、search scan 或 high-offset 性能。
...
| Activity | 四个权威持久来源 + SQLite transaction 内 summary/filter/stable page + exact kind/UUID detail + durable Analysis Run/bounded events/exact result + final-page download/recording/analysis retryability 已有；real-Major completed、一次 startup interrupted→new-run retry、Production/filter/reload exact selection 与双尺寸已过 | cancel/heartbeat、并发 retry、真实多页与大库性能、虚拟化、其他 kind mutation 与两种外部 retry 的真实失败任务门禁 |
...
1. **Player persistent projection、atomic evidence 与 explicit compare（已实现）**：Analysis completion 原子写入 current Player-match projection；SQLite 完成目录 filter/sort/count/page 并返回 coverage，不再运行时扫描 1,000 个 Analysis 文档。点击单场玩家进入 kills/deaths/weapons/duels/utility/objectives/highlights 原子证据，并可 Watch、跳 Round/Replay、加入制作；全局 Player profile 另以 exact participant query 显示 first-10 持久证据、总数、索引完整性与 Round/Replay/完整 Search。全局目录最多保存两个 URL-owned Steam64 进行 ordered exact compare，Reload/Back 恢复 single/pair 与窄窗 Inspector。fresh `playerprojection-audit` 已验证 M1/M2/M3 coverage `3/3`、三条比赛日期不可用的 local rows、FalleN/NiKo pair 和双尺寸无 overflow。下一步仍是真实多页/大库、multi-match 图表和 Watch 外部动作门禁。
...
5. **Activity Center（生产纵切已实现，Analysis interrupted→retry 门禁已过）**：统一读取分析、下载、录制与导出权威事实，已有 search/kind/state、SQLite 边界稳定分页以及独立于 list window 的 exact kind/UUID endpoint。URL selection、strict response binding、last-good stale/backoff/404 unavailable、abort guard、Production deep link 与 mutation lease 已落。fresh Tauri/真实 M1 已证明 Production/filter/reload exact selection 和 Activity 2560×1392/1100×700；真实 M3 已证明一次 startup `interrupted`、new-run retry completed 与旧 run 保留。`fc80c5f` 另修复权威状态前进后 queued receipt 陈旧，Web 619 passed / 3 skipped。failed/cancelled Steam download 与 recording suffix retryability、pre-persist fail-closed 已过 TDD，但外部 failure/retry 仍未实跑。下一步是 Analysis cancel/heartbeat/并发 retry、其他 kind 真实 mutation、Steam/recording failure-retry、多页/大库性能与通用 pause/resume。
6. **Advanced Analysis（进行中）**：Weapons、Duels、Utility、Openings、Clutch Review、Team Round、Team Economy 与 Man Advantage Review 已落。Man Advantage 真实 M1 21/21 verified、矩阵 `7/3/1/10`、首次领先方胜 17/负 4、5 个 lead-change rounds，并通过最大化、1100×700 与 drawer；它严格是 roster minus canonical death targets，不是 health/alive、win probability、trade、KAST 或 rating。Team Round 真实 M1 2×2 矩阵和 kill/round_end Inspector 双尺寸验收已过，Clutch Review 真实 M1 1100×700 为 20 attempts/0 wins。Team Economy 真实 M1 的四格 purchase/cost/item oracle、top-3+remainder、50 行详情在最大化通过；agent-browser 首次发现 1100×700 overlap，修复与 TDD 后已复验为单一纵向滚动、Inspector 可达且无横向溢出。M2 R16 m0NESY `1v2` 仍只有 TDD/既有 real oracle，未在 currentaudit UI 载入；Watch 未跑。仍缺 equipment value/economy type/freeze-time advantage、持久 Team entity，Utility 的页面内双尺寸验收也仍未完成。
...
- 已闭合 Analysis exact-context/recovery 纵切：Activity 在 SQLite 边界执行 summary/filter/stable page，并以 exact kind/UUID endpoint 独立读取权威 row；URL selection、Production deep link、filter-out/reload 保持、observer backoff/404/abort 与 mutation guard 已落。旧 gate 证明 completed six-event/result；新 fresh M3 gate 证明 run `4ded…` startup interrupted、retry 新建 distinct completed `02a7…` 且旧 run 保留。`fc80c5f` 修复 stale queued receipt，final screenshot 只确认 completed exact load `receipt=[]`，same-session 自动退役由 TDD/focused 24 证明。未闭合的是 Analysis cancel/heartbeat/并发 retry、其他 kind mutation、多页/性能；download 与 recording retry 仍只有 readiness/runtime/Web TDD。
...
- Player 单场原子证据、持久目录 projection、最多两名 URL-owned 显式玩家 compare、全局 profile 的 first-10 exact involvement evidence 与三条 truthful local match rows 已接线；Player 双尺寸、未知日期标注和 URL Reload/Back 已用 final build 复验。未完成的是真实多页/大库、multi-match charts/maps/heatmap、Watch 外部动作和持久 Team entity。
