...
use super::stringtables::StringTableBudget;
...
pub struct FirstPassParser<'a> {
    ...
    /// All observed `CMsgPlayerInfo.userid` low bytes keyed by SteamID.
    /// `None` means the same SteamID was observed with conflicting user IDs.
...
impl<'a> FirstPassParser<'a> {
    pub fn new(inputs: &'a ParserInputs<'a>) -> Self {
        Self::with_resource_limits(inputs, usize::MAX, usize::MAX, usize::MAX)
    }

    pub fn with_resource_limits(
        inputs: &'a ParserInputs<'a>,
        max_fullpacket_segments: usize,
        max_decompressed_frame_bytes: usize,
        max_string_table_bytes: usize,
    ) -> Self {
        Self::with_resource_budget(
            inputs,
            max_fullpacket_segments,
            max_decompressed_frame_bytes,
            Arc::new(StringTableBudget::new(max_string_table_bytes)),
        )
    }
    ...
    pub(crate) fn with_resource_budget(
        ...
        max_fullpacket_segments: usize,
        max_decompressed_frame_bytes: usize,
        string_table_budget: Arc<StringTableBudget>,
    ) -> Self {
        FirstPassParser {
            max_fullpacket_segments,
            max_decompressed_frame_bytes,
            ...
            player_userids: BTreeMap::default(),
...
/// Maps a demo file without copying it into owned memory.
///
/// # Safety
///
/// The caller must guarantee that the mapped file is not modified or truncated by any process
/// for the entire lifetime of the returned mapping. Prefer reading untrusted demos into an owned
/// `Vec<u8>` at the application boundary.
pub unsafe fn create_mmap(path: String) -> Result<Mmap, DemoParserError> {
