...
## 10.12 阶段 3g 落地记录 —— 设置、恢复、工作台与引导（2026-08-16）

四次提交。出口条件：`check-web-layers`（999 个文件）/ `typecheck` / `lint` / `build` 退出码 0；vitest 487 文件 4774 用例通过（3f-2 结束时是 4722，本轮 +52）；en-US 目录零缺失（新增 187 条已翻译）；`cargo clippy --workspace --all-targets -- -D warnings` 与 `cargo test --workspace` 全绿。

### 10.12.1 `RecordingDefaults` 三值化（§10 偏离 5）

`mute_voice` + `isolate_target_voice` → `voice: RecordingVoicePolicy`。

两个布尔表达四个状态而只有三个合法，第四个（全部静音**且**只留目标）在 runtime 的 `presentation` 里第一件事就被拒绝，配置校验里再拒一次。**一个装不下非法值的形状不需要任何守卫**，所以两处校验都删掉了——连同 `recordingContract.ts` 里那段优先级说明，它存在的唯一理由就是把这对布尔映射到线上早就在用的枚举。

收口成功从下游看是这样的：**一段优先级规则变成一次字段拷贝。**

### 10.12.2 设置五节

`SECTION_BODY` 从一项加四个 `null` 变成五项。每节装什么由 `AppConfig` **实际带什么**决定，不是照着「设置页通常有什么」写。

每一行都写「影响：…」——这是画板对这一页的原话（「每项写清『会影响什么』」），也是它按用户目标而不是按配置键分组的理由。共享行组件把 `hint` 设成**必填 prop**：一个说不出自己影响什么的行，不该是一个设置项。

**画板画了而「游戏与录制」没有画的两行**，各自把理由写在屏幕上而不是做成禁用控件：

- **录制输出目录**没有独立字段——录制写在 `data_dir` 下，而第二个写同一字段的选择器正是两个控件显示不同值的成因。这一行指向「文件与资料库」。
- **默认视角**任何地方都没有字段。镜头视角是逐镜头的，而画板这一行描述的是 Agent「没有明确依据时」的回退，没有任何存储。画出来就是画一个答案无处可去的控件。

**「导出诊断包」画着、禁用、写明原因**：画板承诺了它，而没有任何路由能打包。`exportAgentSessions` 导出的是会话，是另一回事；把这一页内存里有的东西存下来不是诊断包。

**测试抓到的两处**：`rejectWatchPath` 而不是 `includes` 去重——Windows 上只差大小写或末尾分隔符的两个字符串是同一个文件夹，「移除」会删错行；以及非法的更新源地址在失焦时**保留草稿**——原来会把存储值还原到光标下并带走错误提示，字段看起来像是接受了然后忘了用户输入的东西。

### 10.12.3 Agent 五个开关

3g-be 给了字段（§10.11），所以画了。每个开关写**整份设置文档**，因为路由是替换语义；有一条测试钉住连续改两个开关是叠加而不是后者覆盖前者。

「默认成片时长」用档位而不是滑块（实际答案就是四个数中的一个），而**服务端接受 5…3600**，所以设在档位之外的值会被印出来而不是隐藏——否则面板看起来像是把设置重置了。「左右」两个字带进了 hint：它是目标不是上限，需要更长的方案不会被截断。

### 10.12.4 恢复中心

三张卡对三条路由。每张按同样顺序说三件事：**哪里坏了**（或者没坏，这是正常答案且值得说出来）、**会改什么**（用文件与记录说，不用动词）、**不会动什么**。

第三件是这一页可用的原因。走到恢复中心的人已经在担心丢东西，「清理」不带范围就是按不下去的按钮。

**每个动作都要一次确认，没有例外**，包括看起来像日常维护的那个：「清理未完成的暂存输出」删的是失败的导出留下的东西，而那正是用户可能还想救它的时刻。

**清理会报告失败数**，不只报告成功数：留下三个被占用的文件的清理没有清理完，只印 `deleted` 会让用户再按一次。`scan_limited` 同理。

### 10.12.5 工作台首页余下四块

五块齐了。每一块此前都在等定义它形状的数据文件，三轮才凑齐。

**只画线上带的东西**：方案行印镜头数与证据条数，**不印**画板的「预计 42 秒」和「Kael · Mirage」主体行——前者要为每个方案再取一次完整 body 才能求和（而 summary 故意不带镜头），后者根本没有字段（§10.5 缺口 1）。

**环境提示**照画板那一句话做：「环境问题只在阻塞相应任务时出现在这里」。什么都没阻塞时保持沉默，**包括检查还在飞的时候**；只对 `missing` 与 `blocked` 出现，`warning` 不出现；每一行在服务端原话旁边写**它让什么做不了**，因为「未探测到 HLAE」不告诉任何人录制起不来。

**无法识别的检查状态按「不阻塞」处理**，和设置里那个状态点的做法**相反**且是有意的：一个分不出状态的点保持中性因为它在**描述**，而这一块在**打断用户**，对一个没人定义过的状态打断，是横幅变成噪音的方式。

**测试抓到的两处**：方案块加载时整块消失，导致五个查询在五个时刻落地时页面跳两次——现在标题在所有分支之外，只有主体切换；以及 `Notice` 上的 `data-home-block` 没有生效，它的 props 是封闭集合、不透传其余属性。

### 10.12.6 `/guide` 与三步提示条

画板 02 建议**下线** GuidePage（「引导建议改成首次使用时的三步提示条」），§10 决定保留。两半都做了：

- **三步提示条**在工作台，只在资料库为空时出现；
- **`/guide`** 保留提示条装不下的那件事：**环境自检**，逐项，带重新检查。

旧的 `features/guide/GuidePage.tsx` 198 行正是这个：一份 `quickCheck` 读数加一排入口卡。下线这一页会把读数一起下线，而「为什么录制起不来」是新用户第一天就有的问题。

**「首次使用」的判定是资料库为空，不是标记位。** 存标记在两个方向上都错：用户清空数据、真的从头开始时它仍是已忽略；而用了一年的人换台机器它又出现。资料库为空就是这个提示条真正关心的条件，不需要存储、不需要迁移、不会不同步。这也意味着**没有「不再显示」按钮**，这是对的取舍——可忽略的提示条需要记住忽略，而它要记的事情数据已经能回答。

**它与设置 · 高级与诊断的区别**：那一节按原始状态逐项列出，给已经有问题的人诊断用。这一页回答「我现在能做什么」：同样的检查，按它启用什么分组，每一项写清没有它时**还能做什么**。缺 HLAE 只挡录制，新用户需要知道今天仍然可以导入和分析。

### 10.12.7 路由表与外壳回填

`/guide` 的注册按 §10.6 缺口 15 的教训**一次改完四处**：`ROUTE_PATHS` / `createAppRoutes` / 侧栏前缀规则 / 命令面板，加上 composition root 的绑定表。

顺带修了一个假保障：`pageSkeleton.test.tsx` 的「覆盖每个 §7 目的地」断的是**数量**，加一条路由不会被发现——3g 加 `/guide` 时就是这样漏过去的。改成对照 `ROUTE_PATHS` 时撞上 §2.1 规则 3（`pages/**` 不得 import `app/**`），**分层检查是对的**，所以真正的交叉检查放在 `src/routes.test.tsx`（composition root，本来就同时看两边），页表那边保留计数并写明为什么。

**外壳回填**：设置的 CS2 位置 / 数据目录 / 监听目录都接上了 `chooseDirectories` 与 `openDirectory`；编辑器素材库的「导入」接上了 `chooseFiles` → `importMediaAsset`。**逐个顺序导入**：每次导入都要探测文件，二十个并发只是把二十个探测排到一个一次答一个请求的服务上；第一个失败就停下并报告——静默导入十个里的七个，会让用户数行数才发现。

### 缺口

1. **「重新定位」仍然禁用。** 它需要 `relinkMediaAsset` **加上**一个决定：已经引用旧文件的片段怎么办。那是修复流程不是选择器。理由写在按钮上。
2. **「稍后处理」没有画。** 方案模型里没有任何 snooze——没有 dismissed_at、没有提醒——所以这个按钮要么什么都不做，要么把行藏到刷新为止，都比不画更糟。
...
## 10.15 缺口清零记录 —— 剩下的六项（2026-08-17）

§10.14 收尾时列了四项「需要产品决定」和两项「可做」。六项全做了，其中**一项是我判断错了**。

### 10.15.1 导出诊断包 —— 路由一直都在

按钮画着、禁用着，理由写的是「服务端还没有打包诊断信息的接口」。**那句话是错的。** `POST /api/app/diagnostics/export` 一直存在，`productApi.exportDiagnostics` 也一直存在——整个 `shared/desktop/product.ts` 没有任何消费者，所以没人发现。

报告是 `数据目录/diagnostics` 下的一份 JSON：版本、运行会话、系统与架构，以及哪些东西**已配置**——从不含凭据值，从不含媒体。页面**读 `contains_secrets` 而不是断言它**，因为这份文件用户马上要交给别人，那个承诺应该来自造它的服务。另一条分支有测试钉着。

成功提示是一句话不是 `Notice`：每条通知按设计都带一个恢复动作，而一份已经写好的报告没有什么可恢复的。失败才是 `Notice`，带「重试」。

### 10.15.2 重新定位 —— 卡点不是我以为的那个

我写的是「要决定已经引用这个文件的片段怎么办」。查完发现**片段根本不需要处理**：`relink_media_asset` 保留 `id`、`project_id`、`created_at`，而片段引用的是 `asset_id`，所以每个片段自动跟到新文件。

真正的问题是**长度**，而且这条链上三层都不检查：

| 层 | 检查了什么 |
| --- | --- |
| `relink_media_asset` | proxy 是否在生成、路径是否被人改过 |
| `EditorProject::validate` | 工程内部一致性，看不到素材 |
| `validate_editor_clip`（导出） | `source_out > source_in` 就通过 |

于是 relink 到一个更短的文件，导出会让 `FFmpeg` 去读文件末尾之外——出来的是短片段或黑帧，**不是任何人能指着说的错误**。

所以路由拒绝，消息里带上两个时长，面板原样印出来而不是换成「重新定位没有完成」。容差是 60fps 下的一帧：同一素材的两次编码在最后一位小数上就会不一致。

**未探测到的时长不阻断修复**：对「不知道」拒绝，会让这个修复恰好在文件最难判断的时候不可用，而那正是一个丢失素材通常所处的状态。

这把「重新定位」限定回它的字面意思——**同一个文件，移动了**。真换一条素材是 `replace_asset_upload`，它不受这条约束，也是另一个还不存在的按钮。

### 10.15.3 稍后处理 —— 一个瞬间，不是一个标记

`snoozed_until` 由**客户端**算：「今天不再提醒」指的是读者**自己**的下一个本地午夜，服务端不知道他的时区。23:50 推迟的方案十分钟后就回来，那正是这句话的意思，也是它不是「24 小时」的原因。

**它不是 `Archived`。** 归档是永久的「不做了」；被推迟的方案仍在等待确认，并且会自己回来——所以没有一个「已忽略」列表需要用户去里面翻。

`snooze_agent_plan` **刻意不是一次编辑**：不动 revision、不写 origin、不碰镜头。推迟是关于一个列表的偏好，把它当编辑会让每个打开的编辑器以为方案被改过，也会和同一时刻的真实编辑撞车。有测试钉住这一点。

过滤在面板里做而不是在查询里：一个瞬间已过的行会在下次读取时自己回来，不需要任何东西去清标记。

### 10.15.4 默认视角 —— 找到读它的人才让字段有意义

记录写的是「任何地方都没有字段」。枚举是有的（`AgentShotView` 逐镜头，检视器可以改），缺的是一个存下来的回退值，而画板那一行描述的正是它。

**让这个字段有意义的是找到读它的人。** 除了 `agentHandoff.ts`，没有任何生产代码构造 `AgentPlanShot`；它为每条交接过来的高光铸一个镜头，并且把 `view: 'observer'` 写死，注释说在这里选择「would be guessing at a creative decision it has no input for」。**这个设置就是那个 input。**

`kind` 保持写死并说明原因：镜头类型有七个成员，是逐镜头依据证据选的，给它们一个统一默认值会是比那句注释描述的更糟的猜测。

设置路由是替换语义，所以新键是必填——**两个既有测试用失败证明了这一点**。其中一个原本断言「`default_video_seconds` 越界返回 400」，如果不补字段它会继续通过，但是因为错误的原因。

### 10.15.5 方案摘要的总时长，以及 `shot_count` 数错了

`total_duration_seconds` 算在镜头体已经解开的地方——存储层的查询本来就要 decode `shots_json` 去数镜头，求和不额外花钱。客户端求和意味着渲染一个十行列表要再发十个请求。

做这件事时撞出一处分歧：`shot_count` 是 `shots.len()`，**包含软删除的镜头**——那些用户拿掉、留在文档里好让「撤销」有东西可还原的镜头。录制路由排除它们，方案条排除它们，用户从这一行点开的页面把它们画成删除线。只有列表行数了它们。**现在两个数字都排除。**

`AgentPlanSummary` 因此失去 `Eq`：`f64` 没有全序相等。它在测试里被比较，从不做键。

主体行（「Kael · Mirage」）仍然没画：方案与 Demo、选手之间根本没有绑定，那是 §10.5 缺口 1，是模型改动而不是加个字段。

### 10.15.6 `CapturedPlan.kind` 收成枚举

四个值全部由 `tools.rs` 的四个工具处理器铸出，第五个意味着一个新处理器。桌面那一跳也带上枚举，于是 `validate_proposal` 里那条回答「unknown proposal kind」的分支消失了：它现在是穷尽的，**第五种 kind 会在决定如何校验它的地方变成编译错误**。

### 结论

**没有剩下的缺口了。** 记录在案、有理由不做的还有两条，各自的理由写在代码里：编辑器导出固定 `auto`/85（画板没有导出对话框），以及 `speed_segments` 不可编辑（画板只画了一个速度字段，而变速斜坡是曲线 UI）。

阶段表上只剩阶段 5：`design/` 回流到一个新建的 Claude Design 设计系统项目。
