...
export type UtilityEvidenceActionContext = {
...
export type UtilityEvidenceActionContract = {
...
export function utilityEvidenceActionContract(
  workspace: AnalysisWorkspace,
  evidence: UtilityAtomicEvidence,
  context: UtilityEvidenceActionContext,
): UtilityEvidenceActionContract {
  ...
  const addReason = playerId === null
    ? 'A verified utility actor is required before this evidence can be added to production.'
  ...
  const compilation = playerId === null
    ...
    : {
        ...
        title: [
          'Utility',
          evidence.utility_type.toLocaleUpperCase(),
          evidence.actor_name,
          `Round ${evidence.round}`,
        ].filter(Boolean).join(' · '),
