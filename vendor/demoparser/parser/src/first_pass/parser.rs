...
pub struct FirstPassOutput<'a> {
    ...
    pub settings: &'a ParserInputs<'a>,
    pub baselines: Arc<AHashMap<u32, Vec<u8>>>,
    pub prop_controller: &'a PropController,
    pub cls_by_id: &'a Vec<Class>,
    pub qfmap: &'a QfMapper,
    pub ge_list: &'a AHashMap<i32, DescriptorT>,
    ...
    pub player_userids: Arc<BTreeMap<u64, Option<i32>>>,
    pub added_temp_props: Vec<String>,
    pub wanted_players: AHashSet<u64>,
    pub header: AHashMap<String, String>,
    pub order_by_steamid: bool,
    pub list_props: bool,
    pub(crate) string_table_budget: Arc<crate::first_pass::stringtables::StringTableBudget>,
}
#[derive(Debug)]
pub struct Frame {
    pub tick: i32,
...
impl<'a> FirstPassParser<'a> {
    ...
    pub fn resize_if_needed(buf: &mut Vec<u8>, needed_len: Result<usize, snap::Error>) -> Result<(), DemoParserError> {
        Self::resize_if_needed_with_limit(buf, needed_len, usize::MAX)
    }

    pub fn resize_if_needed_with_limit(buf: &mut Vec<u8>, needed_len: Result<usize, snap::Error>, limit: usize) -> Result<(), DemoParserError> {
        match needed_len {
            Ok(len) => {
                if len > limit {
                    return Err(DemoParserError::ResourceLimitExceeded {
    ...
    pub fn create_first_pass_output(&self) -> Result<FirstPassOutput<'_>, DemoParserError> {
        ...
        Ok(FirstPassOutput {
            ...
            baselines: Arc::new(self.baselines.clone()),
            ...
            string_tables: Arc::new(self.string_tables.clone()),
            stringtable_players: Arc::new(self.stringtable_players.clone()),
            player_userids: Arc::new(self.player_userids.clone()),
            added_temp_props: self.added_temp_props.clone(),
            list_props: self.list_props,
            string_table_budget: Arc::clone(&self.string_table_budget),
        })
    }
    fn fallback_if_first_pass_missing_data(&mut self) -> Result<(), DemoParserError> {
        if !self.fullpacket_offsets.contains(&HEADER_ENDS_AT_BYTE) {
            if self.fullpacket_offsets.len() >= self.max_fullpacket_segments {
                return Err(DemoParserError::ResourceLimitExceeded {
                    resource: "fullpacket segments",
                    limit: self.max_fullpacket_segments,
                    actual: self.fullpacket_offsets.len() + 1,
                });
            }
...
mod resource_limit_tests {
    ...
    #[test]
    fn decompression_buffer_rejects_claim_above_limit_before_resize() {
        let mut buffer = Vec::new();
        let error = FirstPassParser::resize_if_needed_with_limit(&mut buffer, Ok(65), 64).expect_err("oversized frame");
...
impl<'a> FirstPassParser<'a> {
    ...
    fn handle_short_header(&mut self, _file_len: usize, bytes: &[u8]) -> Result<(), DemoParserError> {
        if bytes.len() < 16 {
        ...
        let _file_length_expected = match bytes[8..12].try_into() {
