...
impl<'a> SecondPassParser<'a> {
    pub fn parse_packet_ents(&mut self, bytes: &[u8], is_fullpacket: bool) -> Result<(), DemoParserError> {
        ...
        for _ in 0..msg.updated_entries() {
            let entity_delta = bitreader.read_u_bit_var()?;
            let entity_delta = i32::try_from(entity_delta).map_err(|_| DemoParserError::MalformedMessage)?;
            entity_id = entity_id
                .checked_add(1)
                .and_then(|id| id.checked_add(entity_delta))
                .filter(|id| *id >= 0 && *id <= 100_000)
    ...
    pub fn parse_paths(&mut self, bitreader: &mut Bitreader) -> Result<usize, DemoParserError> {
        ...
        */
        ...
        loop {
            // Huffman lookup permits zero-padded lookahead at the tail, but the selected
            // code length is still checked by consume_checked against real remaining bits.
            let peeked_bits = bitreader.peek_padded_nbits(HUFFMAN_CODE_MAXLEN)?;
            let (symbol, code_len) = self
                .huffman_lookup_table
                .get(peeked_bits as usize)
                .copied()
                .ok_or(DemoParserError::MalformedMessage)?;
            if !(1..=HUFFMAN_CODE_MAXLEN).contains(&(code_len as u32)) {
                return Err(DemoParserError::MalformedMessage);
            }
            bitreader.consume_checked(code_len as u32)?;
    ...
    fn create_new_entity(&mut self, bitreader: &mut Bitreader, entity_id: &i32, _events_to_emit: &mut Vec<GameEventInfo>) -> Result<(), DemoParserError> {
        ...
        // Insert baselines
        if let Some(baseline_bytes) = self.baselines.get(&cls_id).or_else(|| self.baseline_seed.get(&cls_id)) {
