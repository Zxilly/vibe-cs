...
describe('EmptyState presets', () => {
  ...
  const cases: readonly [EmptyStatePreset, string][] = [
    ['no-matches', '还没有比赛'],
    ['not-analysed', '这场还没分析'],
    ['no-hits', '没有命中的证据'],
    ['no-outputs', '还没有输出'],
  ...
  it.each(cases)('renders the artboard copy for %s', (preset, title) => {
    const html = renderMarkup(<EmptyState preset={preset} actions={ACTION} />);
  ...
  it('draws a glyph only where the artboard draws one', () => {
    expect(renderMarkup(<EmptyState preset="no-matches" actions={ACTION} />)).toContain('<svg');
    expect(renderMarkup(<EmptyState preset="not-analysed" actions={ACTION} />)).not.toContain('<svg');
  });

  it('carries the folder description of 还没有比赛 verbatim', () => {
    const html = renderMarkup(<EmptyState preset="no-matches" actions={ACTION} />);
...
describe('EmptyState structure', () => {
  it('names the region by its heading', () => {
    const html = renderMarkup(<EmptyState preset="no-outputs" actions={ACTION} />);
  ...
  it('takes the heading level from the caller so it slots into the page outline', () => {
    expect(renderMarkup(<EmptyState preset="no-outputs" actions={ACTION} headingLevel={2} />)).toContain('<h2');
  });

  it('paints the error tone with the fail border and fail ink, not with colour alone', () => {
    const html = renderMarkup(<EmptyState preset="error" actions={ACTION} />);
  ...
  it('keeps the empty tone on the plain divider frame', () => {
    const html = renderMarkup(<EmptyState preset="no-hits" actions={ACTION} />);
  ...
  it('lets a page supply its own live conditions in place of the preset copy', () => {
    const html = renderMarkup(
      <EmptyState preset="no-hits" description="当前条件：选手 Kael ＋ 穿墙 ＋ 近 7 天。" actions={ACTION} />,
    );
  ...
  it('works with no preset at all, and omits the description paragraph', () => {
    const html = renderMarkup(<EmptyState title="没有输出" actions={ACTION} />);
