...
use crate::first_pass::stringtables::StringTableBudget;
...
impl EventBudget {
    ...
    pub(crate) fn claim(&self) -> Result<(), DemoParserError> {
        let result = self
            .claimed
            .fetch_update(Ordering::Relaxed, Ordering::Relaxed, |claimed| (claimed < self.limit).then_some(claimed + 1));
        match result {
            Ok(_) => Ok(()),
            Err(claimed) => {
                ...
                Err(DemoParserError::ResourceLimitExceeded {
                    ...
                    limit: self.limit,
                    actual: claimed.saturating_add(1),
                })
            }
        }
    }

    pub(crate) fn check(&self) -> Result<(), DemoParserError> {
...
#[derive(Debug)]
pub(crate) struct VoiceDataBudget {
    limit: usize,
    claimed: AtomicUsize,
}

#[derive(Debug)]
pub(crate) struct InventoryDataBudget {
    limit: usize,
    claimed: AtomicUsize,
}

#[derive(Debug)]
pub(crate) struct CollectedRowBudget {
    limit: usize,
    claimed: AtomicUsize,
}

impl CollectedRowBudget {
    pub(crate) fn new(limit: usize) -> Self {
        Self {
            limit,
            claimed: AtomicUsize::new(0),
...
pub struct SecondPassParser<'a> {
    ...
    pub stringtable_players: Arc<BTreeMap<i32, UserInfo>>,
    pub net_tick: u32,
    ...
    pub string_tables: Arc<Vec<StringTable>>,
    ...
    pub baseline_seed: Arc<AHashMap<u32, Vec<u8>, RandomState>>,
    ...
    pub(crate) event_budget: Arc<EventBudget>,
    pub(crate) string_table_budget: Arc<StringTableBudget>,
    pub(crate) voice_data_budget: Arc<VoiceDataBudget>,
    pub(crate) inventory_data_budget: Arc<InventoryDataBudget>,
    pub(crate) collected_row_budget: Arc<CollectedRowBudget>,
}
#[derive(Debug, Clone)]
pub struct Teams {
...
impl<'a> SecondPassParser<'a> {
    ...
    pub fn new(
        ...
        Self::with_resource_limits(first_pass_output, offset, parse_all_packets, start_end_offset, usize::MAX, usize::MAX)
    }
    ...
    pub fn with_resource_limits(
        ...
        Self::with_resource_budget(
            ...
            parse_all_packets,
            start_end_offset,
            Arc::new(EventBudget::new(max_game_events)),
            max_decompressed_frame_bytes,
            false,
            Arc::new(VoiceDataBudget::new(usize::MAX)),
            Arc::new(InventoryDataBudget::new(usize::MAX)),
            Arc::new(CollectedRowBudget::new(usize::MAX)),
        )
    }
    ...
    pub(crate) fn with_resource_budget(
        ...
        max_decompressed_frame_bytes: usize,
        parse_voice: bool,
        voice_data_budget: Arc<VoiceDataBudget>,
        inventory_data_budget: Arc<InventoryDataBudget>,
        collected_row_budget: Arc<CollectedRowBudget>,
    ) -> Result<Self, DemoParserError> {
        first_pass_output
            .settings
            .wanted_player_props
        ...
        Ok(SecondPassParser {
            ...
            event_budget,
            voice_data_budget,
            inventory_data_budget,
            collected_row_budget,
            string_table_budget: Arc::clone(&first_pass_output.string_table_budget),
            ...
            net_tick: 0,
            c4_entity_id: None,
            stringtable_players: Arc::clone(&first_pass_output.stringtable_players),
            is_debug_mode: debug,
            ...
            baseline_seed: Arc::clone(&first_pass_output.baselines),
            baselines: AHashMap::default(),
            string_tables: Arc::clone(&first_pass_output.string_tables),
            teams: Teams::new(),
...
mod event_budget_tests {
    ...
    #[test]
    fn shared_budget_caps_total_claims_across_threads() {
        let budget = Arc::new(EventBudget::new(100));
        let successful_claims: usize = std::thread::scope(|scope| {
            let handles: Vec<_> = (0..8)
                .map(|_| {
                    let budget = Arc::clone(&budget);
                    scope.spawn(move || (0..50).filter(|_| budget.claim().is_ok()).count())
                })
                .collect();
            handles.into_iter().map(|handle| handle.join().expect("budget worker")).sum()
        });

        assert_eq!(successful_claims, 100);
        assert_eq!(
            budget.check(),
            Err(DemoParserError::ResourceLimitExceeded {
                resource: "game events",
    ...
    #[test]
    fn retained_payload_budgets_are_aggregate_and_fail_before_push() {
        let voice = VoiceDataBudget::new(3);
        assert!(voice.claim(2).is_ok());
        assert_eq!(
            voice.claim(2),
            Err(DemoParserError::ResourceLimitExceeded {
                resource: "voice data bytes",
        ...
        let inventory = InventoryDataBudget::new(3);
        assert!(inventory.claim(2).is_ok());
        assert_eq!(
            inventory.claim(2),
            Err(DemoParserError::ResourceLimitExceeded {
                resource: "inventory data bytes",
    ...
    #[test]
    fn shared_row_budget_caps_parallel_player_collection() {
        let budget = Arc::new(CollectedRowBudget::new(100));
        let successful_claims: usize = std::thread::scope(|scope| {
            let handles: Vec<_> = (0..8)
                .map(|_| {
                    let budget = Arc::clone(&budget);
                ...
                .collect();
            handles.into_iter().map(|handle| handle.join().expect("row budget worker")).sum()
        });
        ...
        assert!(CollectedRowBudget::new(0).claim().is_err());
    }
}
