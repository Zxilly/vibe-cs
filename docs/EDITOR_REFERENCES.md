...
- Wheel input belongs to the whole Timeline panel, including the ruler and track headers, rather than only the clip viewport. The Windows default is horizontal; `Ctrl` temporarily scrolls tracks vertically. The chosen axis stays locked even after it reaches an edge and never falls through to the other axis. `Page Up` and `Page Down` move one visible page left and right. `+`/`=` and `-` zoom, while the bottom zoom scroll bar pans from its centre, zooms from either edge, and fits the whole sequence when fully expanded.
