...
import {
  ...
  writeSequenceTabs,
} from '../domain/editing/sequenceTabs';
import {
  planSourceMediaEdit,
  replaceTimelineClipSource,
  resolveSourceMediaFit,
  type SourceMediaFitMode,
} from '../domain/editing/sourceMediaEditing';
import {
  sameTimelineClip,
  trimRippleClip,
  type TimelineCrossTrackMovePlan,
} from '../domain/editing/timelineEditing';
import {
...
} from '../domain/editing/timelineInteraction';
import {
...
export function ProjectWorkspacePage() {
  ...
  const sendToAgent = async (message: string, agentSelectedClipId: string | null) => {
  ...
  const agentPanel = (
    <AgentPanel
      ...
      onOpenExternalUrl={nativeShell.openExternalUrl}
      confirming={appendAgentEntry.isPending || startRecording.isPending || exportProject.isPending}
      projectId={current.id}
      projectRevision={current.revision}
      onConfirmRecording={async (toolCallId, confirmation) => {
        if (confirmation.projectId !== current.id || confirmation.baseRevision !== current.revision) return;
        ...
        await startRecording.mutateAsync({
          ...
          clipIds: confirmation.clipIds,
        });
      }}
      onConfirmExport={async (toolCallId, confirmation) => {
        if (confirmation.projectId !== current.id
          || confirmation.baseRevision !== current.revision
          || deliveryGatePending
          || currentDeliveryGate?.ready !== true) return;
      ...
      onReturnDelivery={async (groupId, feedback, agentSelectedClipId) => {
        await appendToolDecision(
          deliveryDecisionToolCallId(groupId),
          'rejected',
          t`已退回这组 Agent 修改。修改意见：${feedback}`,
        );
        await sendToAgent(t`退回修改意见：${feedback}`, agentSelectedClipId);
      }}
      onDirectEdit={(groupId, agentSelectedClipId) => {
