...
- Project Media's `从 Demo 创建剪辑` action opens Demo Library with the current Project target;
  choosing an analysed Demo continues directly into Replay selection. A human marks source In and
  Out around the chosen player, then creates one Planned Timeline Clip in that Project through the
  same `ProjectPatch` path used by every other manual edit. The selected tick rate determines the
