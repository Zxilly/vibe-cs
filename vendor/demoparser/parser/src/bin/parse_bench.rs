...
//!
//! Usage: parse_bench <demo.dem> [iters=5] [mode=both|st|mt] [threads=available<=8]

use parser::first_pass::parser_settings::ParserInputs;
use parser::parse_demo::{Parser, ParserResourceOptions, ParsingMode};
...
fn run_once(mmap: &[u8], huf: &Vec<(u8, u8)>, mode: ParsingMode, rayon_threads: usize) -> f64 {
    ...
    .expect("parser resource options");
    let t = Instant::now();
...
fn bench(
    mmap: &[u8],
    huf: &Vec<(u8, u8)>,
    make_mode: impl Fn() -> ParsingMode,
    label: &str,
    iters: usize,
    rayon_threads: usize,
) {
    // one warm-up (page cache / branch predictors) excluded from stats
    let _ = run_once(mmap, huf, make_mode(), rayon_threads);
    let mut times: Vec<f64> = Vec::with_capacity(iters);
    for _ in 0..iters {
        times.push(run_once(mmap, huf, make_mode(), rayon_threads));
    }
    times.sort_by(|a, b| a.partial_cmp(b).unwrap());
    let min = times[0];
    let med = times[times.len() / 2];
    let max = *times.last().unwrap();
    println!("{label:>4}: threads {rayon_threads}  min {min:.3}s  median {med:.3}s  max {max:.3}s  (n={iters})");
}
...
fn main() {
    ...
    let rayon_threads = env::args()
        .nth(4)
        .and_then(|value| value.parse().ok())
        .unwrap_or_else(|| ParserResourceOptions::default().rayon_threads);
    ...
    if mode == "st" || mode == "both" {
        bench(&mmap, &huf, || ParsingMode::ForceSingleThreaded, "ST", iters, rayon_threads);
    }
    if mode == "mt" || mode == "both" {
        bench(&mmap, &huf, || ParsingMode::ForceMultiThreaded, "MT", iters, rayon_threads);
    }
}
